...
<!--        <div ng-repeat="(k,skuinfo) in allskuinfos.bar track by $index" style="width:450px;height: 460px;display: inline-block;" id="barchart{{k}}" on-finish-bar>-->
<!---->
<!--        </div>-->
        <ng-echarts class="echarts" ec-config="pieConfig"
                    ec-option="setbaroption('<?= $kpi ?>','<?= $lastrate ?>',skuinfo.relation.id,'<?= $min ?>','<?= $max ?>',skuinfo.system.id,skuinfo.platform.id,skuinfo.cityLevel.id)"
                    style="width:410px;height: 460px;display: inline-block;"
        ...
        <ng-echarts class="echarts" ec-config="pieConfig"
                    ec-option="setbaroption('<?= $kpi ?>','<?= $lastrate ?>',skuinfo.relation.id,'<?= $min ?>','<?= $max ?>',skuinfo.system.id,skuinfo.platform.id,skuinfo.cityLevel.id,skuinfo.skuname.id)"
                    style="width:410px;height: 460px;display: inline-block;"
