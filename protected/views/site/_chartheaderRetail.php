...
<div style="text-align: center;height: 95px">
    <?php
    if($btnleft) echo BSHtml::button($btnleft,array('id'=>'dpTotal','con'=>"比较",'ng-click'=>"kpibtnchg('$kpi',0)",'class'=>"{{history.$kpi==0 || history.$kpi==2?'active':''}}"));
    if($btnright) echo BSHtml::button($btnright,array('id'=>'dp3','con'=>'趋势','ng-click'=>"kpibtnchg('$kpi',1)",'class'=>"{{history.$kpi==1?'active':''}}"));
    ?>
