...
?>
<div style="height:523px;text-align: center;" class="chart-box" ng-if="visitab=='average_selling_price' && history.average_selling_price==0">
    <ng-echarts ng-repeat="skuinfo in bardata1 track by $index" class="echarts"
                ng-if="(iscapacity == 'unchecked' && isbottle == 'unchecked')"
                ng-init="setdoublebaroption(skuinfo.groupname,$index)"
                style="width:470px;height: 460px;display: inline-block;"
                ec-config="pieConfig"
                ec-option="doublebaroptionlist[$index]">
    </ng-echarts>
    <!--        选择了容量分级-->
    <ng-echarts ng-repeat="skuinfo in bardata1 track by $index" class="echarts"
