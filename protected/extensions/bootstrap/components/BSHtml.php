...
class BSHtml extends CHtml {
    ...
    public static $formLayoutHorizontalLabelClass = 'col-md-2';
    public static $formLayoutHorizontalControlClass = 'col-lg-6 col-md-8 col-sm-10';
    public static $formLayoutHorizontalOffsetClass = 'col-md-offset-2';
    ...
    public static function timeField($name, $value = '', $htmlOptions = array()) {
        return self::textInputField('time', $name, $value, $htmlOptions);
    }
...
//        }
    ...
    public static function image($src, $alt = '', $htmlOptions = array()) {
        ...
        if (yii::app()->params['demomode']) {
            if (yii::app()->urlManager->urlFormat == 'path')
                $file = Yii::getPathOfAlias('webroot') . '/' . $src;

            if (!file_exists($file) && !strpos($src, 'qlogo'))
                $src = 'http://xundian.cntoge.com/' . $src;
            else 
                $src =  '/' . $src;
        }
     ...
     * @return string the generated button group.
     */
    public static function buttonGroup(array $buttons, $htmlOptions = array(),$linkbutton=true) {
        if (!empty($buttons)) {
            ...
            foreach ($buttons as $buttonOptions) {
                ...
                if (!empty($items)) {
                    ...
                    if ($linkbutton || (!empty($type) && $type = self::BUTTON_TYPE_LINK))
    ...
    public static function rowThumbnails(array $thumbnails, $htmlOptions = array()) {
        if (empty($thumbnails))
            return false;
    ...
    public static function thumbnails(array $thumbnails, $htmlOptions = array()) {
        if (!empty($thumbnails)) {
            ...
            $defaultSpan = \bootstrap\helpers\BSArray::popValue('span', $htmlOptions, "col-md-3");
            $output = self::openTag('ul', $htmlOptions);
            foreach ($thumbnails as $thumbnailOptions) {
                ...
                $thumbnailOptions['itemOptions']['class'] = \bootstrap\helpers\BSArray::popValue('span', $thumbnailOptions, $defaultSpan);
    ...
    public static function thumbnailLink($content, $url = '#', $htmlOptions = array()) {
        ...
        $htmlOptions = array_merge($htmlOptions, $itemOptions);
        self::addCssClass('thumbnail', $htmlOptions);
        //       print_r($htmlOptions); print_r($itemOptions);
...
//        $output .= '</li>';
     ...
     * @return string the generated progress bar.
     */
    public static function progressBar($width = 0, $htmlOptions = array(), $autocolor = false) {
