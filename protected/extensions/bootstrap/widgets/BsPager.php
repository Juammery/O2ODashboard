 ...
 */
class BsPager extends CBasePager {
     ...
     */
    public $nextPageLabel = '<span class="glyphicon glyphicon-forward"></span>';

    /**
     * @var string the text label for the previous page button.
     */
    public $prevPageLabel = '<span class="glyphicon glyphicon-backward"></span>';

    /**
     * @var string the text label for the first page button.
     */
    public $firstPageLabel = '<span class="glyphicon glyphicon-fast-backward"></span>';

    /**
     * @var string the text label for the last page button.
     */
    public $lastPageLabel = '<span class="glyphicon glyphicon-fast-forward"></span>';
        ...
        if ($this->nextonly) {
            ...
            if (!$this->hideFirstAndLast) {
                $links[] = $this->createPageLink(
                        $this->lastPageLabel, $pageCount - 1, $currentPage >= $pageCount - 1, false
                );
            }
        }
    ...
    protected function createPageLink($label, $page, $disabled, $active) {
        return array(
            'label' => $label,
            'url' => $disabled ? "":$this->createPageUrl($page),
            'disabled' => $disabled,
            'active' => $active,
        );
    }
